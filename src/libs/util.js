...
export const setAccessToken = (accessToken) => {
    Cookies.set(ACCESS_TOKEN_KEY, accessToken, { expires: cookieExpires || 1 })
}

export const setOrganizationId = (organizationId) => {
    Cookies.set(ORGANIZATION_ID_KEY, organizationId, { expires: cookieExpires || 1 })
}
...
export const getOrganizationId = () => {
    const organizationId = Cookies.get(ORGANIZATION_ID_KEY)
    if (organizationId) {
...
export const showTitle = (item, vm) => {
    let { title, __titleIsFunction__, src } = item.meta
    if (src) return title
    if (!title) return
    if (useI18n) {
 ...
 * @description id
 */
export const setSourceOrganizationIdInLocalstorage = id => {
    localStorage.sourceOrganizationId = id
}
/**
 * @returns id
 */
export const getSourceOrganizationIdFromLocalstorage = () => {
    return localStorage.sourceOrganizationId || 'none'
}
...
export function isAuth (key) {
    return (this.$store.state.user.access || '[]').indexOf(key) !== -1 || false
}

/**
 * 是否有权限 逻辑层使用
 * @param _this 当前调用位置的对象
 * @param key
 * @returns {boolean}
 */
export function isAuthForScript (_this, key) {
    return (_this.$store.state.user.access || '[]').indexOf(key) !== -1 || false
